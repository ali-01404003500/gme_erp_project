                        ...
                        <div class="col-sm-11">
                            <div class="mt-40 mb-50">
                                ...
                                <form action="{{ route('legal.legal-entries.store', app()->getLocale()) }}" method="POST"
                                            ...
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    ...
                                                    <select name="legal_type" id="legal_type"
                                                        ...
                                                        <option value="case" {{ old('legal_type', 'case') == 'case' ? 'selected' : '' }}>Case</option>
                                                        <option value="notice" {{ old('legal_type') == 'notice' ? 'selected' : '' }}>Notice</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <div id="caseFields" class="{{ old('legal_type', 'case') == 'case' ? '' : 'd-none' }}">
                                                <div class="row">
                                                    <div class="col-md-3">
                                                        <div class="form-group">
                                    ...
                                    <!-- ========== HAJIRA INFORMATION - MULTIPLE DATE ========== -->
                                    <fieldset class="border p-2">
                                        <legend class="float-none w-auto p-2">
                                            Hajira Information (Date)
                                        </legend>
                                        <div class="row">
                                            <div class="col-md-4">
                                                <div class="form-group">
                                                    <label for="hajira_date">Hajira Date</label>
                                                    <input type="text" name="hajira_date_temp" id="hajira_date" class="form-control flatdate">
                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label for="hajira_description">Hajira Description</label>
                                                    <textarea name="hajira_description_temp" id="hajira_description" class="form-control" rows="1"></textarea>
                                                </div>
                                            </div>
                                            <div class="col-md-2">
                                                <div class="form-group">
                                                    <label>&nbsp;</label><br>
                                                    <button type="button" class="btn btn-sm btn-primary" id="addHajiraRow">Add Hajira</button>
                                                </div>
                                            </div>
                                        </div>

                                        <div class="form-group">
                                            <table class="table table-bordered" id="hajira_table">
                                                <thead>
                                                    <tr>
                                                        <th>Sl</th>
                                                        <th>Hajira Date</th>
                                                ...
                                                <tbody>
                                                    @if(old('hajira_date'))
                                                        @foreach(old('hajira_date') as $index => $date)
                                                        <tr>
                                                            <td>{{ $loop->iteration }}</td>
                                                            <td>
                                                                {{ $date }}
                                                                <input type="hidden" name="hajira_date[]" value="{{ $date }}">
                                                            </td>
                                                            <td>
                                                                {{ old('hajira_description')[$index] ?? '' }}
                                                                <input type="hidden" name="hajira_description[]" value="{{ old('hajira_description')[$index] ?? '' }}">
                                                            </td>
                                                            <td><button type="button" class="btn btn-sm btn-danger" onclick="removeHajiraRow(this)">Remove</button></td>
                                                        </tr>
                                                        @endforeach
                                                    @endif
                                                </tbody>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" name="convict_father_name"
                                                        id="convict_father_name" class="form-control px-15 mt-2"
                                            ...
                                            <div class="col-md-3">
                                                <div class="form-group" style="margin-top: 32px;">
                                                    <button type="button" class="btn btn-sm btn-primary" id="addConvictRow">Add</button>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <fieldset class="border p-2">
                                        ...
                                        <fieldset class="border p-2" id="witnessSection">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-3">
                                                    <div class="form-group" style="margin-top: 32px;">
                                                        <button type="button" class="btn btn-sm btn-primary" id="addWitnessRow">Add</button>
                                                    </div>
                                                </div>
                                            </div>
...
<script>
    $(document).ready(function() {
        let convictSerial = $('#convict_table tbody tr').length + 1;
        ...
        $('#addConvictRow').on('click', function() {
            ...
            const row = `<tr>
                <td>${convictSerial++}</td>
                <td>${name}<input type="hidden" name="convict_name[]" value="${name}"></td>
                <td>${designation}<input type="hidden" name="convict_designation[]" value="${designation}"></td>
                <td>${phone}<input type="hidden" name="convict_phone[]" value="${phone}"></td>
                <td>${fatherName}<input type="hidden" name="convict_father_name[]" value="${fatherName}"></td>
                <td>${motherName}<input type="hidden" name="convict_mother_name[]" value="${motherName}"></td>
                <td>${nid}<input type="hidden" name="convict_nid[]" value="${nid}"></td>
                <td>${address}<input type="hidden" name="customer_id[]" value="${customer_id}"><input type="hidden" name="convict_address[]" value="${address}"><input type="hidden" name="father_or_husband[]" value="${fatherOrHusband}"></td>
                <td><button type="button" class="btn btn-sm btn-danger" onclick="removeConvictRow(this)"><i class="fa fa-trash"></i></button></td>
            </tr>`;
...
<script>
    $(document).ready(function() {
        let witnessSerial = $('#witness_table tbody tr').length + 1;
        ...
        $('#addWitnessRow').on('click', function() {
            ...
            const row = `<tr>
                <td>${witnessSerial++}</td>
                <td>${name}<input type="hidden" name="witness_name[]" value="${name}"></td>
                <td>${phone}<input type="hidden" name="witness_phone[]" value="${phone}"></td>
                <td>${father}<input type="hidden" name="witness_father_name[]" value="${father}"></td>
                <td>${mother}<input type="hidden" name="witness_mother_name[]" value="${mother}"></td>
                <td>${address}<input type="hidden" name="witness_address[]" value="${address}"></td>
                <td><button type="button" class="btn btn-sm btn-danger" onclick="removeWitnessRow(this)"><i class="fa fa-trash"></i></button></td>
            </tr>`;
...
<!-- ========== HAJIRA SCRIPT (MULTIPLE DATE) ========== -->
<script>
    $(document).ready(function() {
        let hajiraSerial = $('#hajira_table tbody tr').length + 1;

        $('#addHajiraRow').on('click', function() {
            let date = $('#hajira_date').val();
            let desc = $('#hajira_description').val();

            if (!date) {
                toastr.error('Hajira Date is required');
                return;
            }

            let row = `<tr>
                <td>${hajiraSerial++}</td>
                <td>${date}<input type="hidden" name="hajira_date[]" value="${date}"></td>
                <td>${desc}<input type="hidden" name="hajira_description[]" value="${desc}"></td>
                <td><button type="button" class="btn btn-sm btn-danger" onclick="removeHajiraRow(this)">Remove</button></td>
            </tr>`;

            $('#hajira_table tbody').append(row);

            $('#hajira_date').val('');
            $('#hajira_description').val('');
        });
    });

    function removeHajiraRow(btn) {
        $(btn).closest('tr').remove();
        $('#hajira_table tbody tr').each(function(idx) {
