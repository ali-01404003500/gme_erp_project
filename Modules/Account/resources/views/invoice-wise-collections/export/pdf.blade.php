...
<head>
    ...
    <title>Invoice Wise Collection Receipt - {{ $collection->invoice_wise_collection_id }}</title>
    <style>
        body {
...
<body>
    <div class="invoice-container">
        ...
        <header class="my-header">
            @include('partials._for_pdf_header_with_qr', ['qrCode' => true, 'qrCodeUrl' => route('account.collections.invoice-wise-collections.show', $collection->id)])
        </header>

        <!-- Title -->
        <section class="title">
            <h4>Invoice Wise Money Receipt</h4>
        </section>
            ...
            <div class="left">
                <table>
                    <tr>
                        ...
                        <td style="width: 5%;">:</td>
                        <td style="width: 65%;">{{ $collection->invoice_wise_collection_id }}</td>
                    </tr>
                    <tr>
                        <th>Customer Name</th>
                        <td>:</td>
                    ...
                    <tr>
                        ...
                        <td>:</td>
                        <td>
                            <span class="badge badge-{{ $collection->status === 'approved' ? 'success' : ($collection->status === 'verified' ? 'warning' : ($collection->status === 'denied' ? 'danger' : 'secondary')) }}">
            ...
            <div class="right">
                <table>
                    <tr>
                        ...
                        <td style="width: 5%;">:</td>
                        <td style="width: 55%;">{{ \Carbon\Carbon::parse($collection->created_at)->format('d-M-Y') }}</td>
                    </tr>
                    <tr>
                        <th>Created By</th>
                        <td>:</td>
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th style="width: 20%;" class="text-right">Collection Amount</th>
                    </tr>
                </thead>
                <tbody>
                    @php $sn = 1; @endphp
                    @forelse ($collection->salesOrders as $salesOrder)
                        <tr>
                            <td class="text-center">{{ $sn++ }}</td>
                            <td>{{ \Carbon\Carbon::parse($salesOrder->date)->format('d-M-Y') }}</td>
                            <td>{{ $salesOrder->sales_order_id ?? 'N/A' }}</td>
                            <td class="text-right"> {{ number_format($salesOrder->net_amount ?? 0) }}</td>
                            <td class="text-right"> {{ number_format($salesOrder->payments->sum('amount')) }}</td>
                            <td class="text-right"> {{ number_format($salesOrder->net_amount - $salesOrder->payments->sum('amount')) }}</td>
                            <td class="text-right"> {{ number_format($salesOrder->pivot->amount ?? 0) }}</td>
                        </tr>
                    @empty
                        <tr>
        ...
        <section class="invoice-details">
            ...
            <table>
                ...
                <tbody>
                    @php $sn = 1; @endphp
                    @forelse ($collection->payments as $payment)
                        <tr>
                            <td class="text-center">{{ $sn++ }}</td>
                            <td>
        ...
        <section class="total-section clearfix">
            <div class="left">
                <p><strong>IN WORD:</strong> {{ convert_number($collection->total_amount) }} Taka Only</p>
            </div>
            <div class="right">
                ...
                <div class="signature-box left">
                    <div class="signature-display">
                        @if (@$collection->signature->signature)
                            <img src="{{ @$collection->signature->signature }}" alt="Customer Signature">
