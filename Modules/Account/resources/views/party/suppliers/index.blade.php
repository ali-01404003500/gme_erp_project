            ...
            <div class="widget-box widget-color-white ui-sortable-handle clearfix" id="widget-box-7">
                <div class="widget-header widget-header-small">
                    ...
                    <div class="widget-toolbar">
                        <a href="{{ route('account.acc-suppliers.index') }}" ><i class="fa fa-refresh"></i> Refresh</a>
                    </div>

                    <div class="widget-toolbar">
                        <a href="{{ route('account.acc-suppliers.create') }}" ><i class="fa fa-plus"></i> Create</a>
                    </div>
                            ...
                            <tbody>
                                @foreach($suppliers as $item)
                                    <tr>
                                        ...
                                        <td class="pl-3 text-right">{{ number_format($item->opening_balance) }}</td>
                                        <td class="text-center">
                                            <div class="btn-group btn-corner">
                                                @include('partials._user-log', ['data' => $item])

                                                @if ((hasPermission("account-suppliers.edit", $slugs)))
                                                    <a href="{{route('account.acc-suppliers.edit', $item->id)}}" class="btn btn-primary btn-xs"><i class="fa fa-pencil-square"></i></a>
                                                @endif

                                                @if ((hasPermission("account-suppliers.delete", $slugs)))
                                                    <a href="#" onclick="delete_item('{{ route('account.acc-suppliers.destroy', $item->id) }}')" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i></a>
                                                @endif
                                            </div>
                                        </td>
                                    </tr>
