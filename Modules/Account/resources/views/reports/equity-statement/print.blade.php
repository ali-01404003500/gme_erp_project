    ...
    <div class="row">
        ...
        <div class="col-sm-12" style="z-index: 999;">
            <div class="btn-group btn-corner mt-2 pull-right no-print">
                ...
                <a href="{{ route('account.report.transaction-ledger') }}" class="btn btn-danger btn-minier"><i
        ...
        <div class="col-sm-12" style="width:100% !important" id="print">
            ...
            <table border="1" cellpadding="5" style="width: 100% !important; margin-top: 50px; margin-bottom: 20px">
                        ...
                        @foreach($accountControl->accountSubsidiaries as $accountSubsidiary)
                            @foreach($accountSubsidiary->accounts as $account)
                                ...
                                <tr style="font-size: 13px ">
                                    ...
                                    <td>{{ $account->name }}</td>
                                    <td class="text-right pr-1">{{ number_format($account->opening_balance ?? 0) }}</td>
                                    <td class="text-right pr-1">{{ number_format(abs($account->debit ?? 0)) }}</td>
                                    <td class="text-right pr-1">{{ number_format($account->credit ?? 0) }}</td>
                ...
                <tr>
                    ...
                    <th style="text-align: right">{{ number_format(abs($totalOpeningBalance)) }}</th>
                    <th style="text-align: right">{{ number_format(abs($totalDebit)) }}</th>
                    <th style="text-align: right">{{ number_format($totalCredit) }}</th>
                    <th style="text-align: right">{{ number_format(($totalOpeningBalance + $totalDebit + $totalCredit)) }}</th>
                </tr>
                </tbody>
