...
@section('description', 'Receive Vouchers')
@section('page-head')
    <style type="text/css">
        .bg-qty {
            background: #5759604a;
                        ...
                        <div class="breadcrumb-main__wrapper">
                            <div class="action-btn mt-sm-0 mt-15 d-flex align-items-center">
                                @if (hasPermission('account.voucher-receives.create'))
                                    <a href="{{ route('account.voucher-receives.create') }}" class="btn px-20 btn-primary btn-sm">
            ...
            <div class="row">
                <div class="col-md-12" style="padding-bottom: 20px">
                    <div class="row" style="width: 100%">
                        <div class="col-md-6">
                            <h4 class="text-capitalize breadcrumb-title">{{ trans('Receive Vouchers List') }}</h4>
                        </div>
                    </div>
                </div>
                <div class="col-md-12">
...
<td class="text-center">{{ ($vouchers->currentPage() - 1) * $vouchers->perPage() + $loop->iteration  }}</td>
                                                        ...
                                                        <td class="text-center">
                                                            <div class="btn-group btn-corner">
                                                                ...
                                                                <a href="{{ route('account.voucher-receives.show', $item->id) }}" target="_blank" class="btn btn-success btn-xs" title="View Details"><i class="fa fa-eye"></i></a>
                
                                                                @if(!$item->is_approved)
                
                                                                    @if(hasPermission("account.voucher-receives.approve"))
                                                                        <a href="{{ route('account.voucher-receives.edit', $item->id) }}?type=approve" target="_blank" class="btn btn-success btn-xs" title="Approve Voucher">
                                                                            <i class="fa fa-check"></i></a>
                                                                    @endif
                
                                                                    @if(hasPermission("account.voucher-receives.update"))
                                                                        <a href="{{route('account.voucher-receives.edit', $item->id)}}" class="btn btn-primary btn-xs" title="Edit">
                                                                            <i class="fa fa-edit"></i></a>
                                                                    @endif
                                                                @endif
                
                                                                @if(hasPermission("account.voucher-receives.destroy"))
                                                                    <button type="button" data-action="{{ route('account.voucher-receives.destroy', $item->id) }}" class="btn btn-danger btn-xs delete-confirm" title="Delete"><i class="fa fa-trash"></i></button>
                                                                @endif
                                                            </div>
                                                        </td>
                                                    </tr>
