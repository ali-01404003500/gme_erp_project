    ...
    <div class="row">
        ...
        <!-- heading -->
            <div class="widget-box widget-color-white ui-sortable-handle clearfix" id="widget-box-7">
                    ...
                    <div class="widget-toolbar border smaller" style="padding-right: 0 !important">
                        <div class="pull-right tableTools-container" style="margin: 0 !important">
                            <div class="dt-buttons btn-overlap btn-group">
                                <a href="{{route('account.account-subsidiaries.index')}}"
                ...
                <!-- INPUTS -->
                <form action="{{route('account.account-subsidiaries.update', $accountSubsidiary->id)}}" method="post">
                    @csrf @method('put')
                    <div class="row" style="width: 100%; margin: 0 0 20px !important;">
                        <div class="col-sm-12 px-4">
                            <!-- Account Group -->
                            @include('Account::includes.inputs.option-select', ['modelVariable' => 'accountGroups', 'is_required' => true, 'edit_id' => $accountSubsidiary->account_group_id])

                            <!-- Account Controls -->
                            @include('Account::includes.inputs.option-select', ['modelVariable' => 'accountControls', 'is_required' => true, 'edit_id' => $accountSubsidiary->account_control_id])
                            ...
                            <!-- Status -->
                            @include('Account::includes.inputs.status', ['edit_id' => $accountSubsidiary->status])
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#account_group_id').change(function () {
                $.get(`{{route('account.ajax.account-controls')}}?account_group_id=${$(this).val()}`, function (res) {
