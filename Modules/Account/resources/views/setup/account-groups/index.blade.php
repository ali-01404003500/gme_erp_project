...
@section('description', 'Account Groups List')
@extends('layout.app')
@section('content')
    <!-- CONTENT AREA -->
            ...
            <div class="row">
                <div class="col-md-12" style="padding-bottom: 20px">
                    <div class="row" style="width: 100%">
                        <div class="col-md-6">
                            <h4 class="text-capitalize breadcrumb-title">{{ trans('Account Groups') }}
                            </h4>
                        </div>
                    </div>
                </div>
...
<td class="text-center">{{ ($accountGroups->currentPage() - 1) * $accountGroups->perPage() + $loop->iteration  }}</td>
                                            ...
                                            <td class="text-center">
                                                <div class="btn-group btn-group-sm" role="group"
                                                    ...
                                                    @if (hasPermission('account.account-setup.account-groups.edit'))
                                                        <button type="button"
                                                            data-action="0"
                                                            data-data="{{ $accountGroup }}"
                                                            class="btn btn-outline-primary btn-edit" data-toggle="tooltip"
                                                            data-placement="top" title="Edit" data-bs-toggle="modal"
                                                            @if($accountGroup->is_deletable == 0) disabled @endif
                                                    ...
                                                    @if (hasPermission('account.account-setup.account-groups.destroy'))
                                                        <button type="button"
                                                            data-action="0"
                                                            @if($accountGroup->is_deletable == 0) disabled @endif
        ...
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header" id="createModalLabel">
                    <h5 class="modal-title">Create Account Group</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
                </div>
                <form action="{{ route('account.account-groups.store') }}" method="post" id="createForm">
                    @csrf
