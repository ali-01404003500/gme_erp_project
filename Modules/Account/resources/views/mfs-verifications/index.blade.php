 @section('title', 'MFS Verification')
@section('description', 'MFS Verification')
@extends('layout.app')
                        ...
                        <div class="breadcrumb-action justify-content-center flex-wrap">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">MFS Verification</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                                            ...
                                            <tr>
                                                ...
                                                <td width="10%">
                                                    <select name="status" class="form-control "> 
                                                        @if (hasPermission('account.mfs-verifications.check'))
                                ...
                                <tbody>
                                    ...
                                    @foreach ($entries as $entry)
                                        <tr>
                                            ...
                                            {{-- Particulars --}}
                                            <td> 
                                                <strong>Date:</strong> {{ $entry->transaction_date ?? '-' }} <br>
                                                <strong>No:</strong> {{ $entry->transaction_no ?? '-' }} <br>
    ...
    $(document).on("click", ".status-btn", function() {
        ...
        if (status === "pending" || status === "approved") {
            ...
            $("#statusSubmitBtn").removeClass("btn-danger").addClass("btn-primary").html(
                '<i class="fas fa-check"></i> Confirm bKash Verification'
            );
        } else {
            $("#statusModalTitle").html(
                '<i class="fas fa-times-circle text-danger"></i> Confirm bKash Deny'
            );
            $("#statusSubmitBtn").removeClass("btn-primary").addClass("btn-danger").html(
                '<i class="fas fa-times"></i> Confirm bKash Deny'
            );
        }

        // dynamic form action
        $("#statusForm").attr("action", "{{ route('account.mfs-verifications.update-status', 'ID') }}".replace("ID", id));
    });
