                        ...
                        <div class="pull-right tableTools-container" style="margin: 0 !important">
                            <div class="dt-buttons btn-overlap btn-group">
                                ...
                                @if(hasPermission("fund.transfers.create", $slugs))
                                    <a href="{{route('account.fund-transfers.create')}}" class="dt-button btn btn-white btn-info btn-bold" title="Create New" data-toggle="tooltip" tabindex="0" aria-controls="dynamic-table">
                                        <span>
                                            ...
                                            <div class="btn-group btn-corner">
                                                ...
                                                @if(!$item->is_approved)
                                                    @if(hasPermission("fund.transfers.approve", $slugs))
                                                        <a href="#" onclick="approve_item('{{route('account.fund-transfers.approve.update', $item->id)}}')" class="btn btn-purple btn-xs" title="Approve Fund Transfer"><i class="fa fa-check"></i></a>
                                                    @endif
                                                    @if(hasPermission("fund.transfers.edit", $slugs))
                                                        <a href="{{route('account.fund-transfers.edit', $item->id)}}" class="btn btn-primary btn-xs" title="Edit"><i class="fa fa-pencil"></i></a>
                                                    @endif
                                                    @if(hasPermission("fund.transfers.delete", $slugs))
                                                        <a href="#" onclick="delete_item('{{ route('account.fund-transfers.destroy', $item->id) }}')" class="btn btn-danger btn-xs" title="Delete"><i class="fa fa-trash"></i></a>
                                                    @endif
                                                @endif
                                            </div>
                                        </td>
