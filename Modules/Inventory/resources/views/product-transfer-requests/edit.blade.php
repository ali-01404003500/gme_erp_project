                        ...
                        <div class="breadcrumb-action justify-content-center flex-wrap">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">{{ trans('menu.product-transfer-request-edit-menu-title') }}</li>
                                </ol>
                            </nav>
                        </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    <h4 class="text-capitalize breadcrumb-title">
                        {{ trans('menu.product-transfer-request-edit-menu-title') }}
                    </h4>
                    <x-error-alart />
                </div>
                <div class="col-md-12">
                    <div class="card mb-4">
                        <div class="card-body">
                            <form action="{{route('inv.product-transfer-requests.update', $productTransferRequest->id)}}" method="POST" id="product_transfer_form"
                                ...
                                <div class="row mb-4">
                                            ...
                                            <select name="source_branch_id" class="form-control tom-select source_branch"
                                                ...
                                                @foreach ($branches as $branch)
                                                    <option value="{{ $branch->id }}"
                                                        {{ old('source_branch_id', $productTransferRequest->source_branch_id) == $branch->id ? 'selected' : '' }}>
                                                        {{ $branch->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <select name="destination_branch_id" class="form-control tom-select"
                                                ...
                                                @foreach ($branches as $branch)
                                                    <option value="{{ $branch->id }}"
                                                        {{ old('destination_branch_id', $productTransferRequest->destination_branch_id) == $branch->id ? 'selected' : '' }}>
                                                        {{ $branch->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <textarea name="remarks" id="remarks" cols="30" rows="2" class="form-control">{{ old('remarks', $productTransferRequest->remarks) }}</textarea>
                                        </div>
                                    </div>
                                                        ...
                                                        @if ($product_catalog_ids)
                                                            @for ($i = 0; $i < count($product_catalog_ids); $i++)
                                                                <tr>
                                                                    ...
                                                                    <td>
                                                                        <button type="button" class="btn btn-danger btn-xs" onclick="removeRow(this)"><i class="fa fa-times"></i></button>
                                                                    </td>
                                                                </tr>
                                                            @endfor
                                                        @else
                                    ...
                                    <div class="col-md-12">
                                        <div class="button-group d-flex pt-25 justify-content-md-end justify-content-start">
                                            ...
                                            @if (hasPermission('inv.product-transfer-requests.approve') && $productTransferRequest->status == "pending" && request()->aprove == 1)
                                                <button type="submit" id="reject" class="btn btn-danger mr-2">
                                            ...
                                            @endif
                                            @if(hasPermission('inv.product-transfers.create') && $productTransferRequest->status == "approved" && auth()->user()->branch_id == $productTransferRequest->source_branch_id )
                                                ...
                                                href="{{ route('inv.product-transfers.create',[ "product_transfer_request_id"=>$productTransferRequest->id]) }}"><i
                                                    class="fas fa-truck"></i> Create Transfer</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
...
@section('page_scripts')
    ...
    <script type="text/javascript">
        ...
        // Initialize available stock for existing items on page load
        $(document).ready(function() {
            $('.select-product').each(function() {
                ...
                if(productId && sourceBranch) {
                    ...
                    $.get("{{ route('inv.stocks.product-available-in-branch') }}?product_id=" + productId + "&branch_id=" + sourceBranch)
                        .then(function(data) {
                            row.find('.avoilable-stock').text(data);
                        });
                }
            });
        });
