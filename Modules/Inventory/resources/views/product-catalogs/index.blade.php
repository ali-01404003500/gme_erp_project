                                ...
                                <tbody>
                                    ...
                                    @foreach ($productCatalogs as $key => $productCatalog)
                                        <tr>
                                            ...
                                            <td class="text-start">{{ $productCatalog->model }}</td>
                                            <td class="text-start">{{ optional($productCatalog->brand)->name }}</td>
                                            <td class="text-end">{{ number_format($productCatalog->mrp,2) }}</td>
                                            <td class="text-start">{{ optional($productCatalog->tag)->name }}</td>
                                            <td class="text-center">
        ...
        $(document).ready(function() {
            ...
            const productSelect = new TomSelect("#name", {
                ...
                load: function(query, callback) {
                    ...
                    $.ajax({
                        url: "{{ route('inv.product-catalogs-autocomplete.product-name') }}",
                        type: "GET",
                        data: { search: query },
            ...
            const productModelSelect = new TomSelect("#model", {
                valueField: "id",
                labelField: "text",
                searchField: [], 
                load: function(query, callback) {
                    ...
                    $.ajax({
                        url: "{{ route('inv.product-catalogs-autocomplete.product-model') }}",
                        type: "GET",
                        data: { search: query },
                        success: function(res) {
                            productModelSelect.clearOptions();
                            callback(res.map(item => ({ id: item.id, text: item.label })));
                        },
                        error: function() {
                            callback();
            ...
            @if(request('model'))
                productModelSelect.addOption({
                    id: "{{ request('model') }}",
                    text: "{{ request('model') }}"
                });
                productModelSelect.setValue("{{ request('model') }}");
            @endif
        });
    </script>
@endsection
