...
class DeliveryService
{
    ...
    public function getAll(int $limit = 20)
    {
        ...
        return Delivery::query()
            ->with(['source', 'deliveryDetails.product']) // ← Eager loading যোগ করুন
            ->whereHas('source')
            ->searchByFields(['status'])
            ->filterByDateRange('delivery_date')
            ->paginate($limit);
    ...
    public function storeShipping(Delivery $delivery)
    {
        // dd($delivery->source);
        /***
    ...
    public function assignDongleToCustomer(DeliveryStock $serialStockSalesOrderDeliveryStock, $customerId)
    {
    ...
    public function update(Delivery $delivery, $data, array $deliveryDetails, $deliveryStockDetails)
    {
        ...
        // OPTIMIZED: load source with details in one query
        $source      = $delivery->source()->with('details')->first();
        $expectedQty = $source->details->sum('quantity');
            ...
            if (! empty($deliveryStockDetails['serial_no'][$product_id])) {
                foreach ($deliveryStockDetails['serial_no'][$product_id] as $key2 => $serialNo) {
                    ...
                    if ($source->customer_id) {
                        $this->assignDongleToCustomer($deliveryStock, $source->customer_id);
                    }
                    $result['deliveryStock'][$deliveryDetail->id][] = $deliveryStock;
                }
            }
        ...
        if ($source->is_shipment == 1 && $source->shipment) {
            $shipment           = $this->storeShipping($delivery);
            $result['shipment'] = $shipment;
        }
    ...
    public function show($id)
    {
        return Delivery::with([
            ...
            'deliveryDetails.deliveryStocks',
        ])->findOrFail($id);
    }
}
