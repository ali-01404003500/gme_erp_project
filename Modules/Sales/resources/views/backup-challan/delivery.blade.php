@section('title', 'Backup/Challan Delivery')
@section('description', 'Backup/Challan Delivery')
@extends('layout.app')
@section('content')
        ...
        <div class="social-dash-wrap">
                            ...
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">
                                        {{ trans('menu.delivery-sales-order-menu-title') }}</li>
                                </ol>
                            </nav>
                        </div>
            ...
            <div class="row">
                <div class="col-md-12 m-2">
                    <h4 class="text-capitalize breadcrumb-title">{{ trans('menu.delivery-sales-order-menu-title') }} ({{$backupChallan->invoice_id}})</h4>
                    <x-error-alart />
                </div>
                <div class="col-md-12">
                    <div class="card mb-4">
                        <div class="card-body">
                            <form action="{{ route('sales.backup-challan-deliveries.store') }}" method="POST">
                                @csrf
                                <input type="hidden" name="backup_challan_id" value="{{ $backupChallan->id }}">
                                <input type="hidden" name="backup_challan_type" value="{{ $backupChallan->type }}">
                                <div class="row mb-4">
                                    <div class="col-md-4 mt-4">
                                        <div>
                                            Customer name: {{ $backupChallan->customer->company_name }}
                                        </div>
                                    </div>
                                    <div class="col-md-4 mt-4">
                                        <div>
                                            Phone No: {{ $backupChallan->customer->phone }}
                                        </div>
                                    </div>
    
                                    <div class="col-md-4 mt-4">
                                        <div>
                                            Invoice Date: {{ $backupChallan->invoice_date }}
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-12 mt-4 t">
                                        <table class="table table-bordered">
                                            ...
                                            <tbody id="productTable">
                                                @foreach ($backupChallan->backupChallanDetails as $backupChallanProduct)
                                                    <tr id="product_{{ $backupChallanProduct->product_id }}">
                                                        <td>
                                                            {{ $backupChallanProduct?->product?->name }}
                                                            <input type="hidden" name="product_id[]" value="{{ $backupChallanProduct->product_id }}">
                                                        </td>
                                                        <td>{{ numberFormat($backupChallanProduct->price) }}</td>
                                                        <td>{{ numberFormat($backupChallanProduct->quantity) }}</td>
                                                        <td><input type="number" name="quantity[]" class="form-control" readonly></td>
                                                        <td>{{ numberFormat($backupChallanProduct->amount) }}</td>
                                                        <td>
                                                            {{-- add button --}}
                                                            <div class="btn-group btn-group-sm">
                                                                <button type="button" class="btn btn-secondary btn-xs btn-add" data-bs-toggle="modal" data-limit="{{numberFormat($backupChallanProduct->quantity)}}" data-bs-target="#select-product-stock-modal" data-url="{{ route('sales.sales-order-deliveries.select-stock', $backupChallanProduct->product_id) }}">
