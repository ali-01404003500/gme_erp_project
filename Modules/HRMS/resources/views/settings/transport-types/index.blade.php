...
@section('description', 'Transport Types')
@extends('layout.app')
@section('content')
    <!-- CONTENT AREA -->
                            ...
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">
                                        {{ trans('menu.hrm-settings-transport-types-menu-title') }}</li>
                                </ol>
                            </nav>
                        </div>
            ...
            <div class="row">
                <div class="col-md-12" style="padding-bottom: 20px">
                    <div class="row" style="width: 100%">
                        <div class="col-md-6">
                            <h4 class="text-capitalize breadcrumb-title">{{ trans('menu.hrm-settings-transport-types-menu-title') }}
                            </h4>
                            <x-error-alart />
                        </div>
                    </div>
                                ...
                                <table id="zero-config" class="table table-bordered grid-table dt-table-hover" data-page='@include('utils.table_paginate', ['data' => $transportTypes])'
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center">Transport Type Name</th>
                                    ...
                                    <tbody>
                                        @foreach ($transportTypes as $key => $transportType)
                                            <tr>
                                        <td class="text-center">{{ ($transportTypes->currentPage() - 1) * $transportTypes->perPage() + $loop->iteration  }}</td>
                                                <td class="text-center">{{ $transportType->code }}</td>
                                                <td class="text-center">{{ $transportType->name }}</td>
                                                <td class="text-center">{{ $transportType->description }}</td>
                                                <td class="text-center">
                                                    @if ($transportType->status == 1)
                                                        <span class="badge badge-success">Active</span>
                                                    @elseif ($transportType->status == 0)
                                                ...
                                                <td class="text-center">
                                                    <div class="btn-group btn-group-sm" role="group"
                                                        ...
                                                        @if (hasPermission('hrm.settings.transport-types.update'))
                                                            <button type="button" data-action="{{ route('hrm.settings.transport-types.update', $transportType->id) }}" data-data="{{$transportType}}" class="btn btn-outline-primary btn-edit" data-toggle="tooltip" data-placement="top" title="Edit"
                                                        ...
                                                        @if (hasPermission('hrm.settings.transport-types.destroy'))
                                                            <button type="button"
                                                                data-action="{{ route('hrm.settings.transport-types.destroy', $transportType->id) }}"
