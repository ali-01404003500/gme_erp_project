                                ...
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active" aria-current="page">
                                            {{ trans('menu.noticeboards-list-menu-title') }}</li>
                                    </ol>
                                </nav>
                            </div>
                                                ...
                                                <td class="text-center">
                                                    <select name="notice_type_id" class="form-control">
                                                        ...
                                                        @foreach ($noticeTypes as $noticeType)
                                                            <option value="{{ $noticeType->id }}" {{ (request('notice_type_id') == $noticeType->id) ? 'selected' : '' }}>{{ $noticeType->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td colspan="2">
                                                    <div class="input-daterange input-group">
                                                        ...
                                                        <input type="text" class="form-control flatdate" name="toDate"
                                                            value="{{ request('toDate') }}" autocomplete="off" placeholder="To Date" />
                                                    </div>
                                                </td>
                                ...
                                <table id="zero-config" class="table table-bordered" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($noticeBoards as $key => $value)
                                            <tr>
                                            <td class="text-center">{{ ($noticeBoards->currentPage() - 1) * $noticeBoards->perPage() + $loop->iteration  }}</td>
                                                <td>{{ $value->title }}</td>
                                                ...
                                                <td class="text-center">
                                                    <div class="btn-group btn-group-sm" role="group"
                                                        ...
                                                        @if (hasPermission('hrm.noticeboards.show'))
                                                            <a class="btn btn-outline-primary" href="{{ route('hrm.noticeboards.show', $value->id) }}"><i class="fas fa-eye"></i></a>
                                                        @endif
