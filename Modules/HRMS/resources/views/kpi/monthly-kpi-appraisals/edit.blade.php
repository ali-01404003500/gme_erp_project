...
@section('title', 'Edit Monthly KPI Appraisal')
@section('description', 'Edit Monthly KPI Appraisal for Employee')
...
<div class="container-fluid">
    <div class="social-dash-wrap">
        ...
        <div class="col-md-12 m-2">
            <h4 class="text-capitalize breadcrumb-title">Edit Monthly KPI Appraisal</h4>
            <x-error-alart />
        </div>
            ...
            <div class="row justify-content-center">
                <div class="col-sm-12">
                    <div class="mt-40 mb-50 p-30">
                        <form action="{{ route('hrm.kpis.monthly-kpi-appraisals.update', $appraisal->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="row">
                                <!-- Employee -->
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" class="form-control" value="{{ $appraisal->employee->full_name }}" readonly>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="month" name="assessment_month" id="assessment_month" class="form-control" 
                                            value="{{ \Carbon\Carbon::parse($appraisal->assessment_month)->format('Y-m') }}" readonly>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" class="form-control" value="{{ $appraisal->employee->employementDetail->branch->name ?? 'N/A' }}" readonly>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" class="form-control" value="{{ $appraisal->employee->employementDetail->department->name ?? 'N/A' }}" readonly>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group mb-3">
                                        ...
                                        <input type="text" class="form-control" value="{{ $appraisal->employee->employementDetail->designation->name ?? 'N/A' }}" readonly>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <tbody>
                                        @foreach ($appraisal->details as $index => $detail)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>
                                                    {{ $detail->responsibility->description ?? 'N/A' }}
                                                    <input type="hidden" name="responsibilities[{{ $index + 1 }}][id]" value="{{ $detail->id }}">
                                                    <input type="hidden" name="responsibilities[{{ $index + 1 }}][responsibility_entry_id]" value="{{ $detail->responsibility_entry_id }}">
                                                    <input type="hidden" name="responsibilities[{{ $index + 1 }}][key_responsibility]" value="{{ $detail->responsibility->description ?? '' }}">
                                                    <input type="hidden" name="responsibilities[{{ $index + 1 }}][target_days]" value="{{ $detail->target_days }}">
                                                    <input type="hidden" name="responsibilities[{{ $index + 1 }}][weight]" value="{{ $detail->weight }}">
                                                </td>
                                                <td><input type="number" step="0.01" class="form-control form-control-sm" value="{{ $detail->target_days }}" readonly></td>
                                                <td><input type="number" step="0.01" name="responsibilities[{{ $index + 1 }}][actual_days]" class="form-control form-control-sm" min="0" value="{{ old('responsibilities.' . ($index + 1) . '.actual_days', $detail->actual_days) }}"></td>
                                                <td><input type="number" step="0.01" name="responsibilities[{{ $index + 1 }}][kpi_score]" value="{{ old('responsibilities.' . ($index + 1) . '.kpi_score', number_format($detail->kpi_score ?? 0)) }}" class="form-control form-control-sm" readonly></td>
                                                                                                <td>{{ $detail->weight }}</td>

                                                <td><input type="number" step="0.01" name="responsibilities[{{ $index + 1 }}][performance_score]" value="{{ old('responsibilities.' . ($index + 1) . '.performance_score', number_format($detail->performance_score ?? 0)) }}" class="form-control form-control-sm" readonly></td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                    <tfoot>
                                        <tr class="table-secondary">
                                            ...
                                            <th><span id="total-performance-score">{{ number_format($appraisal->details->sum('performance_score')) }}</span></th>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
                            ...
                            <div class="table-responsive mt-4">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>70</td>
                                            <td><input type="text" id="performance_score_display" name="achieved_performance_score" class="form-control" value="{{ old('achieved_performance_score', number_format($appraisal->achieved_performance_score)) }}" readonly></td>
                                            <td><input type="text" name="performance_score_note" id="performance_score_note" class="form-control" value="{{ old('performance_score_note', $appraisal->performance_score_note) }}"></td>
                                        </tr>
                                        <tr>
                                            <td>Succession Management</td>
                                            <td>20</td>
                                            <td><input type="number" step="0.01" name="succession_management_score" id="succession_management_score" class="form-control" min="0" max="20" value="{{ old('succession_management_score', $appraisal->succession_management_score ?? 0) }}"></td>
                                            <td><input type="text" name="succession_management_note" id="succession_management_note" class="form-control" value="{{ old('succession_management_note', $appraisal->succession_management_note) }}"></td>
                                        </tr>
                                        <tr>
                                            <td>Behavioral Performance</td>
                                            <td>10</td>
                                            <td><input type="number" step="0.01" name="behavioral_performance_score" id="behavioral_performance_score" class="form-control" min="0" max="10" value="{{ old('behavioral_performance_score', $appraisal->behavioral_performance_score ?? 0) }}"></td>
                                            <td><input type="text" name="behavioral_performance_note" id="behavioral_performance_note" class="form-control" value="{{ old('behavioral_performance_note', $appraisal->behavioral_performance_note) }}"></td>
                                        </tr>
                                        <tr class="table-secondary">
                                            <td><strong>Aggregate Score</strong></td>
                                            <td><strong>100</strong></td>
                                            <td><strong><span id="aggregate_score_display">{{ number_format(($appraisal->achieved_performance_score ?? 0) + ($appraisal->succession_management_score ?? 0) + ($appraisal->behavioral_performance_score ?? 0)) }}</span></strong></td>
                                            <td></td>
                                        </tr>
                                    </tbody>
                                </table>
                                ...
                                <div class="alert alert-info">
                                    <strong>Remarks:</strong> <span id="remarks_display">Loading remarks...</span>
                                </div>
                            </div>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-12">
                                    <div class="form-group mb-3">
                                        ...
                                        <textarea name="notes" id="notes" class="form-control" rows="3">{{ old('notes', $appraisal->notes) }}</textarea>
                                    </div>
                                </div>
                            </div>
                            ...
                            <input type="hidden" name="kpi_template_assign_employee_id" value="{{ $appraisal->kpi_template_assign_employee_id }}">
                        </form>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Generate remarks based on aggregate score
