...
class CustomerController extends Controller
{
    ...
    public function __construct(CustomerService $service, CustomerSettingService $customerSetting)
    {
    ...
    public function create()
    {
        ...
        $data['customerTypes'] = CustomerType::pluck('name', 'id');
        $data['customers']     = Customer::select('id', 'company_name')
            ->orderBy('company_name')
            ->limit(100)
            ->get();
        $data['employees'] = Employee::where('status', 1)
            ->pluck('full_name', 'id');
        $data['areas'] = cache()->remember('areas_list', 3600, function () {
    ...
    public function edit(Customer $customer)
    {
        ...
        $data['employees'] = cache()->remember('employees_for_select', 3600, function () {
            return Employee::where('status', 1)
                ->pluck('full_name', 'id');
        });

        $data['areas'] = cache()->remember('areas_for_select', 86400, function () {
            return Area::select('id', 'area')->get();
        });

        $data['customer'] = $customer->load([ // eager loading for relationships
            'customerType',
            'area',
            'userRef',
            'customer',
