            ...
            <div class="row justify-content-center">
                <div class="col-sm-10">
                    <div class="mt-40 mb-50">
                        <form action="{{ route('cms.document-entries.update', $documentEntry->id ) }}" method="POST"
                            ...
                            <div class="row mb-2">
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-control tom-select" name="document_type_id"
                                        ...
                                        @foreach ($documentTypes as $item)
                                            <option value="{{ $item->id }}" @if (old('document_type_id', $documentEntry->document_type_id) == $item->id) selected @endif>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-control tom-select" name="document_head_id"
                                        ...
                                        @foreach ($documentHeads as $item)
                                            <option value="{{ $item->id }}" {{ old('document_head_id', $documentEntry->document_head_id) == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" class="form-control flatdate " name="date" id="date"
                                        placeholder="Date" value="{{ old('date', $documentEntry->date) }}" required>
                                </div>

                                <div class="form-group col-md-6">
                                    <label for="file">File<span class="text-danger">*</span></label>
                                    <x-file-uploader name="attachment" :value="$documentEntry->attachment" />
                                    {{-- <input id="attachment" type="file" class="file-control form-control"  name="attachment" data-old="{{ $documentEntry->attachment }}"
                                        class="file-control" data-preview-element="front-image-preview"> --}}
                                </div>
                                <div class="form-group col-md-12">
                                    <label for="remarks">Remarks </label>
                                    <textarea name="remarks" id="remarks" class="form-control" placeholder="Remarks">{{ old('remarks', $documentEntry->remarks) }}</textarea>
                                </div>
                            </div>
