...
class CBCLicenseRequisitionService
{
    private $smsService;

    public function __construct(CBCLicenseSmsService $smsService = null)
    {
        $this->smsService = $smsService ?? app(CBCLicenseSmsService::class);
    }
    ...
    {
        $today = date('Y-m-d');
        $customer_count = CBCLicenseRequisition::whereDate(DB::raw('DATE(created_at)'), $today)->count();
        $licensesToday = CBCLicenseRequisition::whereDate(DB::raw('DATE(created_at)'), $today)->where('customer_id', $customer_id)->count();
    ...
    public function store(array $data, array $phones = [])
    {
        ...
        $requisition = CBCLicenseRequisition::create($data);
    ...
    public function update(CBCLicenseRequisition $requisition, array $data, array $phones)
    {
    ...
    public function delete(CBCLicenseRequisition $cBCLicenseRequisition)
    {
        $cBCLicenseRequisition->delete();
    }

    public function show($id)
    {
        return CBCLicenseRequisition::findOrFail($id);
    }
            ...
            try {
                ...
                if ($smsData && !empty($smsData['sms'])) {
                    $smsRecord = [
                        ...
                        'c_b_c_license_requisition_id' => $requisition->id,
    ...
    public function storeFromJsonFile()
    {
        ...
        $message = "CBC License Requisitions import completed. Successfully saved: {$result['saved_count']}";
