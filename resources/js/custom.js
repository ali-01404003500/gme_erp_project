...
const getLP20 = (n) => a[Number(n)] != ""? a[Number(n)] : 'zero';
...
const convertFractionalPart = (fractionStr) => {
    if(Number(fractionStr) === 0) return '';
    let str = 'point ';
    for (let i = 0; i < fractionStr.length; i++) { // Also include the index
        const digit = fractionStr[i];
        if(Number(fractionStr.substring(i)) === 0) break;
        str += getLP20(digit) + ' ';
    }
    return str.trim();
}
...
window.numWords = numWords;


document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll('.num-words').forEach(function (el) {
        const span = document.createElement('span');
        span.style.position = 'absolute';
        span.style.fontSize = '12px';
        span.style.color = 'green';
        span.style.textTransform='capitalize';
        span.textContent = el.value != "" ? numWords(el.value) : "";
        el.parentNode.insertBefore(span, el.nextSibling);
        // if changed
        el.addEventListener('input', function () {
            span.textContent = el.value != "" ?" taka " +numWords(el.value) : "";
        });
    });
});
